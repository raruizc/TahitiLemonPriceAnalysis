...
historical_medellin  <- read_excel("Data/Medellin_Mayorista_2012_2025_Months.xlsx")
str(historical_medellin )

## Transforming as Date ####
historical_medellin  <- historical_medellin  %>% 
  mutate(FECHA = as.Date(paste0(FECHA, "-01"), format = "%Y-%m-%d"))
head(historical_medellin )
tail(historical_medellin )
...
# Merge with database
complete_data <- merge(complete_data, historical_medellin , by = "FECHA", all.x = TRUE)
...
historical_medellin  <- complete_data

maxima <-  historical_medellin  %>%
  group_by(year = format(FECHA, "%Y")) %>%
  slice(which.max(AVERAGE_USD))

minima <- historical_medellin  %>%
  group_by(year = format(FECHA, "%Y")) %>%
  slice(which.min(AVERAGE_USD))

series_plot <- historical_medellin  %>% ggplot() +
...
print(series_plot)
ggsave("Figures/01_Series_Medellin.png", plot = series_plot, width = 12, 
       height = 8, dpi = 300)


## Plotting with minimum and maximum area ####

series_plot_area <- historical_medellin  %>% ggplot() +
...
plot(series_plot_area)
ggsave("Figures/01_Series_Area_Medellin.png", plot = series_plot_area, width = 12, 
       height = 8, dpi = 300)
...
length(historical_medellin $FECHA)

### Transforming the decpib object into a data frame ####
decpib_df <- data.frame(time = historical_medellin $FECHA,
...
ggsave("Figures/02_Decomposition_plot_Medellin.png", plot = combined_plot, width = 12, 
       height = 8, dpi = 300)
...
forecast_plot <- ggplotly(
  autoplot(price_train, color = "#1f77b4") +  # Cor da série de treinamento
    ...
    labs(
      #title = "Price Forecast vs Real Values - Medellín",  # Título do gráfico
...
# Save as HTML
library(htmlwidgets)
saveWidget(forecast_plot, file = "Figures/04_Forecast_Medellin.html")
