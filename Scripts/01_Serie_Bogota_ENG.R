...
historical_bogota <- read_excel("Data/Bogota_Corabastos_2012_2025_Months.xlsx")
str(historical_bogota)

## Transforming as Date ####
historical_bogota <- historical_bogota %>% 
  mutate(FECHA = as.Date(paste0(FECHA, "-01"), format = "%Y-%m-%d"))
head(historical_bogota)
tail(historical_bogota)
...
# Merge with database
complete_data <- merge(complete_data, historical_bogota, by = "FECHA", all.x = TRUE)
...
historical_bogota <- complete_data

maxima <-  historical_bogota %>%
  group_by(year = format(FECHA, "%Y")) %>%
  slice(which.max(AVERAGE_USD))

minima <- historical_bogota %>%
  group_by(year = format(FECHA, "%Y")) %>%
  slice(which.min(AVERAGE_USD))

series_plot <- historical_bogota %>% ggplot() +
...
ggsave("Figures/01_Series_Bogota.png", plot = series_plot, width = 12, 
       height = 8, dpi = 300)

## Plotting with minimum and maximum area ####

series_plot_area <- historical_bogota %>% ggplot() +
...
ggsave("Figures/01_Series_Area_Bogota.png", plot = series_plot_area, width = 12, 
       height = 8, dpi = 300)
...
length(historical_bogota$FECHA)

### Transforming the decpib object into a data frame ####
decpib_df <- data.frame(time = historical_bogota$FECHA,
...
ggsave("Figures/02_Decomposition_Plot_Bogota.png", plot = combined_plot, width = 12, 
       height = 8, dpi = 300)
...
residuals <- residuals(arimaprice)

# Using ggtsdisplay to generate diagnostic graphs
ggtsdisplay(residuals, 
            plot.type = "histogram",  # Chart type
            theme = theme_minimal(),
            ylab = "")  
...
forecast_plot <- ggplotly(
  autoplot(price_train, color = "#1f77b4") +  # Cor da série de treinamento
    ...
    labs(
      #title = "Price Forecast vs Real Values - Bogotá",  # Título do gráfico
...
# Save as HTML
library(htmlwidgets)
saveWidget(forecast_plot, file = "Figures/04_Forecast_Bogota.html")
...
manual_arima_model <- Arima(price_train, order = c(1, 0, 1), seasonal = c(0, 1, 1))
...
ggplotly(
  autoplot(price_train, color = "#1f77b4") +  # Cor da série de treinamento
    ...
    labs(
      title = "Price Forecast vs Real Values",  # Título do gráfico
      subtitle = "Comparison between forecasted and actual prices",  # Subtítulo
      x = "Time",  # Rótulo do eixo X
      y = "Price (USD)",  # Rótulo do eixo Y
      color = "Series"  # Título da legenda
